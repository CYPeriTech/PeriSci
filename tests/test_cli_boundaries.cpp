...
static int run_cmd(const std::vector<std::string>& args,
                   const fs::path& workdir,
                   const std::string* stdin_text,
                   std::string* stdout_text)
{
  // Minimal cross-platform-ish approach:
  // Use temporary files to pass stdin/stdout to the CLI (still OK for tests; the red line is about
  // *run_case/CLI behavior*, not about test harness using files).
...
static bool exists_dataset_anywhere(const fs::path& root)
{
  if (!fs::exists(root))
    return false;
