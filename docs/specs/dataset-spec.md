...
- 作为下游消费方（分析、复现实验、审计、归档）的**稳定接口契约**
...
1. The manifest MUST conform to the single authoritative structure defined in §7.
2. No additional top-level fields are permitted.
3. Required fields MUST NOT be missing.
4. No field type mismatches are permitted.
5. Duplicate fields are not permitted.
...
1. Field groups A/B/C MUST be complete.
2. config_hash MUST be reproducible.
3. generator.tool_version MUST match the corresponding version file.
4. “Orphaned results” MUST NOT exist.
5. Failed runs MUST NOT be mislabelled.
...
1. dataset_version MUST exactly match version/dataset_version.txt.
2. generator.tool_version MUST match version/export_tool_version.txt.
3. Identical dataset_version MUST NOT correspond to different config_hash.
4. code_version MUST exist within code/ or the version reference system.
5. results.files MUST fully match the contents of the results/ directory.
...
1. L1–L3 MUST all pass.
2. No unresolved governance conflicts may exist.
3. No failure status may exist.
4. No violation of §9 red-line rules.
...
> 本章节定义 数据集 在当前阶段（v0.2.x）必须满足的**溯源 与可追溯性保障**，以确保其在未来可以**无歧义演进为完整算例资产**（如可审计研究案例、复现实验包或治理对象）。
> 本节条款若被违反则视为破坏性治理违规，必须提升主版本号。

> This section defines the provenance and traceability guarantees that a dataset MUST satisfy at the current stage (v0.2.x), in order to ensure that it can, in the future, unambiguously evolve into a complete case asset (e.g., an auditable research case, a reproducible experiment package, or a governance object).
...
1. `provenance` MUST be a top-level field of `manifest.json`.
2. `provenance` MUST NOT be embedded within `metadata` or any non-normative field.
3. Provenance information MUST NOT be hidden through renaming or structural nesting.
4. Non-standard field names MUST NOT substitute `provenance`.
