...
PeriSci 需要一个清晰的机制来区分以下三类数值实验对象：
...
PeriSci requires a clear mechanism to distinguish three types of numerically experimental
artifacts:
...
Therefore, the architecture needs to explicitly introduce the numerically experimental system:
...
- Difficulty establishing a stable numerical experiment asset system
...
- Improved reproducibility of numerical experiments
...
The numerical experiment system is defined as:
...
通过引入 `cases/`，PeriSci 的数值实验可复现性被进一步强化。
...
The introduction of `cases/` strengthens the reproducibility of numerical experiments in PeriSci and incorporates it into the long-term stability structure:
...
- Examples are no longer treated as stable numerical experiment assets
- Canonical cases become the core foundation of reproducible numerical experiments
...
This decision clarifies the numerical experiment system of PeriSci:
