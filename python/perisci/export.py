...
def export_dataset(bundle: Union[dict, str], out_dir: str, *, perisci_export: str = "perisci-export") -> dict:
    ...
    """
    exe = resolve_executable(perisci_export)
    if isinstance(bundle, dict):
        out = run_cmd([exe, "--bundle", "-", "--out", out_dir], stdin_text=json.dumps(bundle))
    ...
    out = run_cmd([exe, "--bundle", "-", "--out", out_dir], stdin_text=bundle_text)
    return json.loads(out)
