...
当前 `v0.3.x+` 中，Python 层仍是一个**基于 CLI 的过渡性包装层**。

也就是说，当前 Python 工作流的实际执行路径更接近：

```text
python -> apps -> api -> core
```

但这只是阶段性实现方式，不代表长期架构边界。v0.3.x+ 的主要数值能力生长仍由 C++ examples/API/core 路径牵引。
...
- `run_report` 是观测信息位；在当前 `v0.3.x+` 阶段仍可能为空或保持最小内容，后续可逐步增强
